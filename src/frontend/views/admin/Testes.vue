            ...
            <v-expand-transition>
                <div v-if="dadosIniciais">
                    ...
                    <v-card class="mb-6" elevation="3" border>
                        ...
                        <v-divider v-if="resultadoEsperado"></v-divider>
                        
                        <div class="px-4 py-3 bg-blue-lighten-5 text-blue-darken-4" v-if="resultadoEsperado">
                            <div class="text-caption font-weight-bold text-uppercase mb-1"><v-icon icon="mdi-bullseye-arrow" size="small" class="mr-1"></v-icon>Resultado Esperado</div>
...
const cenarioAtual = ref(null);
const mensagemSetup = ref('');
const resultadoEsperado = ref('');
...
async function selecionarCenario(chave) {
    ...
    // Buscar dados iniciais imediatamente (Preview)
    loading.value = true;
    try {
...
async function executarSimulacao() {
    ...
    try {
        ...
        const res = await fetch('/api/testes/setup', {
            method: 'POST',
            headers: authStore.authHeaders(),
            body: JSON.stringify({ cenario: cenarioAtual.value })
        });
        ...
        mensagemSetup.value = data.mensagem;
        resultadoEsperado.value = data.resultadoEsperado || resultadoEsperado.value;
        resultado.value = data.resultadoExecucao || data.resultado;
        dadosIniciais.value = data.dadosIniciais;
        cidades.value = data.cidades || [];

        if (showSnackbar) showSnackbar('Simulação executada com sucesso!');

    } catch (err) {
        console.error(err);
        if (showSnackbar) showSnackbar('Erro ao simular cenário.', 'error');
    } finally {
        loading.value = false;
    }
}
